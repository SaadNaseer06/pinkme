            ...
            <div
                ...
                <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div class="max-w-xl" style="text-shadow: 0 2px 4px rgba(0, 0, 0, 0.6);">
                        ...
                        <p class="mt-3 text-sm lg:text-base max-w-xl opacity-95">Update details and publish your changes when
                            you are ready.</p>
                    </div>
            ...
            <form id="edit-program-form" method="POST" action="{{ route('programs.update', $program) }}"
                ...
                <div class="space-y-8">
                            ...
                            <div class="grid gap-6 md:grid-cols-2">
                                <div>
                                    ...
                                    <input name="event_date" type="date"
                                        value="{{ old('event_date', optional($program->event_date)->format('Y-m-d')) }}"
                                        class="w-full rounded-xl border border-[#DCCFD8] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#DB69A2] focus:ring focus:ring-[#F8D4E6] focus:ring-opacity-70"
                                        required>
                                    @error('event_date')
                                        <p class="mt-1 text-xs text-[#DB69A2]">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <input name="event_time" type="time"
                                        value="{{ old('event_time', \Carbon\Carbon::parse($program->event_time)->format('H:i')) }}"
                                        class="w-full rounded-xl border border-[#DCCFD8] bg-white px-4 py-3 text-sm outline-none transition focus:border-[#DB69A2] focus:ring focus:ring-[#F8D4E6] focus:ring-opacity-70"
                                        required>
                                    @error('event_time')
                                        <p class="mt-1 text-xs text-[#DB69A2]">{{ $message }}</p>
                                ...
                                <div class="relative">
                                    <select name="status"
                                        ...
                                        <option value="upcoming"
                                            @selected(old('status', $program->status) === 'upcoming')>Upcoming</option>
                                        <option value="ongoing" @selected(old('status', $program->status) === 'ongoing')>Ongoing
                                        </option>
                                        <option value="completed"
                                            @selected(old('status', $program->status) === 'completed')>Completed</option>
                                    </select>
                                    <span
                    ...
                    <section class="rounded-2xl border border-[#E9DCE7] bg-white shadow-sm">
                        <div class="border-b border-[#F1E5EF] px-6 py-5">
                            ...
                            <p class="mt-1 text-sm text-[#6C5B68]">Adjust the contribution target for this program.</p>
                        </div>
                        <div class="px-6 py-6">
                            ...
                            <div class="relative mt-1">
                                ...
                                <input name="program_fund" type="number" min="0" step="0.01"
                                    value="{{ old('program_fund', $program->program_fund) }}" placeholder="5000"
                    ...
                    <section class="rounded-2xl border border-[#E9DCE7] bg-white shadow-sm">
                        <div class="border-b border-[#F1E5EF] px-6 py-5">
                            ...
                            <p class="mt-1 text-sm text-[#6C5B68]">Upload a new banner (optional). Leave empty to keep the
                                current one.</p>
                        </div>
                        <div class="space-y-4 px-6 py-6">
                            <label for="banner-input"
                                ...
                                <div class="flex flex-col items-center gap-3 text-[#6C5B68]">
                                    ...
                                    <p id="banner-file-name" class="text-xs font-medium text-[#DB69A2]">No file selected yet</p>
                                </div>
                            ...
                            @if ($program->banner)
                                <div class="rounded-xl border border-[#E9DCE7] bg-white p-4">
                                    ...
                                    <img src="{{ asset('storage/' . $program->banner) }}" alt="Current banner"
                ...
                <aside class="space-y-6">
                    <div class="rounded-2xl border border-[#E9DCE7] bg-white p-6 shadow-sm">
                        <h3 class="text-base font-semibold text-[#213430]">Before you update</h3>
                        <ul class="mt-4 space-y-3 text-sm text-[#6C5B68]">
                            <li class="flex items-start gap-3">
                                ...
                                Confirm your date and time so patients receive accurate updates.
                            </li>
                            <li class="flex items-start gap-3">
                                <span
                                ...
                                Keep fund goal aligned with sponsor commitments.
                            </li>
                            <li class="flex items-start gap-3">
                                <span
                                ...
                                Update the banner only if you want to replace the existing one.
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="rounded-2xl border border-[#E9DCE7] bg-[#FDF7FB] p-6 shadow-inner">
                        ...
                        <p class="mt-3 text-sm text-[#6C5B68]">Programs marked as <span
                                class="font-semibold text-[#DB69A2]">Ongoing</span> appear at the top of patient dashboards.</p>
                    </div>
                </aside>
            </form>
        </div>
