...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-[#6C5B68]">Join upcoming webinars to learn more about available support.</p>
        </div>
    </div>
        ...
        @forelse($webinars as $webinar)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-[#F1E4EC] p-5 flex flex-col gap-3">
                ...
                @if($registration && $registration->isRegistered())
                    ...
                    <form method="POST" action="{{ route('patient.webinars.cancel', $webinar) }}"
                ...
                @else
                    <form method="POST" action="{{ route('patient.webinars.register', $webinar) }}">
                        @csrf
                        <button type="submit"
