                ...
                <table class="min-w-full text-sm text-left">
                    <thead>
                        <tr class="border-t border-[#e0cfd8]">
                            ...
                            <th class="p-2 text-lg text-[#91848C] font-normal">Claim</th>
                    ...
                    <tbody>
                        @forelse ($registrations as $reg)
                            <tr class="border-t border-[#e0cfd8]">
                                ...
                                <td class="p-2 text-sm text-[#91848C]">
                                    @if ($reg->finance_user_id)
                                        {{ $reg->financeUser?->profile?->full_name ?? 'Claimed' }}
                                    @else
                                        Open
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr class="border-t border-[#e0cfd8]">
                                <td colspan="6" class="p-6 text-center text-[#91848C]">No registrations in your queue yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
