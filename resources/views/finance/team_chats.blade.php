...
@section('content')
    <main class="flex-1"
        data-finance-team-chats-page
        data-inbox-fragment-url="{{ route('finance.team_chats.claimable_fragment') }}">
        <div data-finance-claimable-inbox>
            @include('finance.partials.team_chats_claimable', [
                'financeClaimableRegistrations' => $financeClaimableRegistrations ?? collect(),
            ])
        </div>
        @if ($contacts->isEmpty())
            <div class="max-w-4xl mx-auto bg-[#F3E8EF] rounded-xl shadow-sm p-12 text-center space-y-4">
                ...
                <h2 class="text-xl font-semibold text-[#213430]">No administrator contacts</h2>
                <p class="text-sm text-[#91848C]">There are no admin users to message yet. Use Patient Requests to allocate budget when applications are assigned to you.</p>
            </div>
        @else
            <div class="flex flex-col lg:flex-row lg:max-h-[850px] max-w-8xl mx-auto gap-6">
                ...
                <aside class="w-full lg:w-96 bg-[#F3E8EF] rounded-2xl flex flex-col overflow-hidden shadow-sm">
                    <div class="p-5 border-b border-[#E4D6DF] flex items-center justify-between">
                        <h2 class="text-base font-semibold text-[#213430]">Administrators</h2>
                    ...
                    <div class="flex-1 overflow-y-auto" data-chat-contact-list>
                        @foreach ($contacts as $contact)
                            <a href="{{ route('finance.team_chats', ['contact' => $contact['id']]) }}"
                    ...
                    <header class="flex items-center justify-between px-6 py-4 border-b border-[#E4D6DF]">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-[#91848C]">Administrator conversation</p>
                            </div>
                        </div>
