        ...
        <form method="POST" action="{{ route('finance.invoice.store', $registration) }}" class="space-y-6">
            ...
            <div>
                ...
                @if ($calculatedAmount !== null)
                    <input type="number" name="amount" value="{{ old('amount', $calculatedAmount) }}" step="0.01" min="0.01" max="{{ $calculatedAmount }}" required
                        class="w-full rounded-xl border border-[#DCCFD8] bg-white px-4 py-3 text-[#4C4047] focus:outline-none focus:ring-2 focus:ring-[#9E2469] focus:border-transparent transition">
                    <p class="text-xs text-[#6C5F67] mt-1">Maximum is the patient’s selected program total (${{ number_format($calculatedAmount, 2) }}). You may enter a lower amount if needed.</p>
                @else
