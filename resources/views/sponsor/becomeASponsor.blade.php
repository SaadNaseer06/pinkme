    ...
    <div class="flex-1 flex flex-col">
        <main class="flex-1">
            ...
            @php
                ...
                $featuredMonth =
                    $featuredProgram?->month_label ??
                    ($featuredProgram?->event_date ? Carbon::parse($featuredProgram->event_date)->format('M') : 'TBD');
                $featuredDay =
                    $featuredProgram?->day_label ??
                    ($featuredProgram?->event_date ? Carbon::parse($featuredProgram->event_date)->format('d') : '--');
                $featuredDateLabel =
                    $featuredProgram?->date_label ??
                    ($featuredProgram?->event_date
                        ? Carbon::parse($featuredProgram->event_date)->format('F d, Y')
                        : 'To be announced');
                $featuredTime =
                    $featuredProgram?->time_label ??
                    ($featuredProgram?->event_time
                        ? Carbon::parse($featuredProgram->event_time)->format('h:i A')
                        : 'To be announced');
                $featuredSummary = $featuredProgram
                    ? Str::limit(strip_tags($featuredProgram->description), 160)
                    : 'Select a program above to view its details here.';
                $featuredDetail = $featuredProgram
                    ? Str::limit(strip_tags($featuredProgram->description), 220)
                    : 'Choose a program from the list to learn more about its impact.';
            ...
            <div class="mb-8">
                ...
                @forelse ($upcomingPrograms as $program)
                    @php
                        ...
                        $dayLabel =
                            $program->day_label ??
                            ($program->event_date ? Carbon::parse($program->event_date)->format('d') : '--');
                        $dateLabel =
                            $program->date_label ??
                            ($program->event_date
                                ? Carbon::parse($program->event_date)->format('F d, Y')
                                : 'To be announced');
                        $timeLabel =
                            $program->time_label ??
                            ($program->event_time
                                ? Carbon::parse($program->event_time)->format('h:i A')
                                : 'To be announced');
                    ...
                    <div class="bg-[#F3E8EF] rounded-lg p-4 mb-4 flex items-center justify-between md:flex hidden opacity-60 relative group cursor-not-allowed"
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-[#DB69A2] font-medium uppercase tracking-wide">Full Funding Required
                                </p>
                                <span
                                    class="inline-flex items-center gap-2 px-3 py-1 mt-2 text-xs font-semibold text-white bg-[#DB69A2] rounded-full uppercase">Sponsor
                                    Covers 100%</span>
                            </div>
                        </div>
                        <div class="flex flex-col gap-4 text-sm items-end">
                            <div class="flex flex-col text-right">
                                ...
                                <span
                                    class="text-[#213430] font-medium app-text">{{ $program->fund_label ?? 'To be determined' }}</span>
                            </div>
                    ...
                    <div class="bg-[#F3E8EF] rounded-lg p-3 mb-4 flex items-center justify-between md:hidden flex opacity-60 relative group cursor-not-allowed"
                        ...
                        <div class="flex flex-col gap-2 w-full pl-4">
                            ...
                            <p class="text-[13px] font-light text-[#91848C]">
                                {{ Str::limit(strip_tags($program->description), 110) }}</p>
                            <div class="flex justify-between text-[12px]">
                                <span class="text-[#91848C]">Program Fund</span>
                                <span
                                    class="text-[#213430] font-medium">{{ $program->fund_label ?? 'To be determined' }}</span>
                            </div>
                        ...
                        @else
                            <div id="fundCarousel" class="carousel-container flex overflow-x-auto scrollbar-hide">
                                @foreach ($ongoingPrograms as $program)
                                    @php
                                        $image = asset(
                                            'images/' . $fundImagePool[$loop->index % count($fundImagePool)],
                                        );
                                        $monthLabel =
                                            $program->month_label ??
                                            ($program->event_date
                                                ? Carbon::parse($program->event_date)->format('M')
                                                : 'TBD');
                                        $dayLabel =
                                            $program->day_label ??
                                            ($program->event_date
                                                ? Carbon::parse($program->event_date)->format('d')
                                                : '--');
                                        $dateLabel =
                                            $program->date_label ??
                                            ($program->event_date
                                        ...
                                        $detailPayload = [
                                            ...
                                            'fund_amount' =>
                                                $program->program_fund !== null ? (float) $program->program_fund : null,
                                            'fund_label' => $program->fund_label ?? 'To be determined',
                                        ];
                                    @endphp
                                        ...
                                        <div
                                            ...
                                            <div class="p-4 space-y-4">
                                                ...
                                                <div class="flex justify-between items-center text-sm">
                                                    ...
                                                    <span class="text-[#213430] font-medium app-text">Fully Funded - Sponsor
                                                        Pays 100%</span>
                                                </div>
                                                <div class="flex justify-between items-center text-sm">
                                                    <span class="text-[#91848C] app-text">Program Fund</span>
                                                ...
                                                <button type="button"
                                                    class="w-full py-3 text-[#91848C] font-medium border border-gray-300 rounded hover:bg-[#db69a2] hover:text-white hover:border-none rounded-lg app-main"
            ...
            <div id="programDetailOverlay" data-default-commitment="{{ $featuredCommitment }}"
                ...
                <div class="relative max-w-7xl w-full mx-auto">
                    <div class="bg-white/95 border border-[#E5CADB] rounded-[28px] shadow-2xl relative overflow-hidden">
                        <button type="button"
                            class="absolute top-5 right-5 text-[#B08BA2] hover:text-[#7B5268] transition"
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-[60%_35%] gap-6 p-6 lg:p-8">
                            <div class="bg-white border border-[#E7CEDA] rounded-2xl p-6 shadow-sm">
                                <div
                                    class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 border-b border-[#E7CEDA] pb-6">
                                    <div class="flex items-center gap-4">
                                        <div
                                            class="flex flex-col items-center justify-center w-20 h-20 border-2 border-[#DB69A2] rounded-2xl bg-[#FFF7FB] shadow-sm">
                                            <span class="text-sm text-[#DB69A2]"
                                                data-program-month>{{ $featuredMonth }}</span>
                                            <span class="text-4xl font-bold text-[#DB69A2]"
                                                data-program-day>{{ $featuredDay }}</span>
                                        </div>
                                        <div class="space-y-2">
                                            <h2 class="text-2xl font-semibold text-[#213430] app-main" data-program-title>
                                                {{ $featuredProgram->title ?? 'Select a Program' }}</h2>
                                            <p class="text-[#91848C] text-sm app-text max-w-xl" data-program-summary>
                                                {{ $featuredSummary }}</p>
                                            <span
                                                class="inline-flex items-center gap-2 px-3 py-1 text-xs font-semibold text-white bg-[#DB69A2] rounded-full uppercase tracking-wide">Full
                                                Funding Required</span>
                                        </div>
                                    </div>
                                    <div
                                        class="bg-[#FDF1F7] border border-[#E7CEDA] rounded-xl p-4 text-sm text-[#7B5268] w-full sm:w-60">
                                        <p class="font-medium app-text" data-program-commitment>{{ $featuredCommitment }}
                                        </p>
                                        <p class="mt-3"><span class="font-semibold">Time:</span> <span
                                                data-program-time>{{ $featuredTime }}</span></p>
                                    </div>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                    <div>
                                        <h3 class="text-lg font-semibold text-[#213430] mb-4 app-main">Program Overview
                                        </h3>
                                        <div class="space-y-3">
                                            <div
                                                class="flex items-center gap-3 bg-[#FFF7FB] border border-[#E7CEDA] rounded-xl px-4 py-3 shadow-sm">
                                                <i class="far fa-calendar text-[#DB69A2]"></i>
                                                <div>
                                                    <p class="text-xs uppercase tracking-wide text-[#B08BA2]">Date</p>
                                                    <p class="text-sm text-[#213430]" data-program-date>
                                                        {{ $featuredDateLabel }}</p>
                                                </div>
                                            </div>
                                            <div
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm text-[#213430]" data-program-time>
                                                        {{ $featuredTime }}</p>
                                                </div>
                                            </div>
                                            <div
                                                ...
                                                <div>
                                                    <p class="text-xs uppercase tracking-wide text-[#B08BA2]">Program Fund
                                                    </p>
                                                    <p class="text-sm text-[#213430]" data-program-fund>
                                                        {{ $featuredFundLabel }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div>
                                        <h3 class="text-lg font-semibold text-[#213430] mb-4 app-main">About This Program
                                        </h3>
                                        <div
                                            class="bg-[#FFF7FB] border border-[#E7CEDA] rounded-2xl p-5 leading-relaxed text-sm text-[#91848C] space-y-3 shadow-sm">
                                            <p data-program-description>{{ $featuredDetail }}</p>
                                            <ul class="space-y-2 text-[#213430]">
                                                <li class="flex items-center gap-2"><span
                                                        class="text-[#DB69A2] text-xl">&#8226;</span> Single-sponsor
                                                    initiative with high visibility</li>
                                                <li class="flex items-center gap-2"><span
                                                        class="text-[#DB69A2] text-xl">&#8226;</span> Custom recognition
                                                    across materials</li>
                                                <li class="flex items-center gap-2"><span
                                                        class="text-[#DB69A2] text-xl">&#8226;</span> Comprehensive impact
                                                    reporting</li>
                                                <li class="flex items-center gap-2"><span
                                                        class="text-[#DB69A2] text-xl">&#8226;</span> Dedicated liaison to
                                                    manage program delivery</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <form method="POST" action="{{ route('sponsor.sponsorships.store') }}"
                                class="bg-[#FCEAF4] border border-[#E7CEDA] rounded-2xl p-6 space-y-6 shadow-sm"
                                data-sponsorship-form>
                                @csrf
                                <input type="hidden" name="program_id"
                                    value="{{ old('program_id', $featuredProgramId) }}" data-program-input>
                                ...
                                <div class="bg-white/80 border border-[#E9D2DF] rounded-2xl p-5 space-y-4">
                                    <div>
                                        ...
                                        <p class="text-3xl font-bold text-[#DB69A2]" data-program-fund>
                                            {{ $featuredFundLabel }}</p>
                                        <p class="text-sm text-[#91848C] mt-2 app-text">This amount represents the full
                                            commitment required to deliver the program at no charge to beneficiaries.</p>
                                    </div>
                                    <div
                                        class="bg-[#F6E0ED] border border-[#E1C4D3] rounded-xl p-4 text-sm text-[#7B5268] space-y-2">
                                        ...
                                        <ul class="space-y-2">
                                            @foreach ($investmentHighlights as $point)
                                                <li class="flex items-start gap-2"><span
                                                        class="text-[#DB69A2] mt-1">&#8226;</span><span>{{ $point }}</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                ...
                                <div class="space-y-3">
                                    <label for="sponsorshipAmount"
                                        class="text-sm font-semibold text-[#213430] app-main">Your Contribution</label>
                                    <div class="relative">
                                        <span
                                            class="absolute left-4 top-1/2 -translate-y-1/2 text-[#DB69A2] font-semibold mt-3">$</span>
                                        <input type="number" min="1" step="0.01" id="sponsorshipAmount"
                                            name="amount" value="{{ old('amount', $featuredFundAmount) }}"
                                    ...
                                    <p class="text-xs text-[#91848C]">Adjust this value if you plan to co-sponsor or make a
                                        partial contribution.</p>
                                    @error('amount')
                                        <p class="text-xs text-[#B32020]">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div
                                    class="bg-white/70 border border-[#E9D2DF] rounded-2xl p-4 space-y-2 text-sm text-[#7B5268]">
                                    <h4 class="text-sm font-semibold text-[#213430]">Billing Contact</h4>
                                    <div class="flex items-center gap-3">
                                        <img src="{{ $sponsorContext['logo'] }}" alt="{{ $sponsorContext['name'] }}"
                                ...
                                <div class="flex flex-col sm:flex-row gap-3 pt-2">
                                    <button type="button"
                                        class="w-full px-5 py-3 bg-white border border-[#E7CEDA] text-[#91848C] rounded-xl hover:bg-[#F7EDF4] transition"
                                        data-program-modal-close>
                                        Cancel
                                    </button>
                                    <button type="submit"
                                        class="w-full px-6 py-3 bg-[#DB69A2] text-white rounded-xl shadow-sm hover:bg-[#C4568E] transition">
                                        Confirm Sponsorship
                                    </button>
                                </div>
                            </form>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const defaultCommitment = overlay.dataset.defaultCommitment || (commitmentEl ? commitmentEl
                .textContent : '');
            ...
            function closeModal() {
                ...
                applyProgramData(defaultPayload, {
            ...
            programTriggers.forEach(function(trigger) {
                ...
                trigger.addEventListener('click', function(event) {
                    ...
                    openOverlayWithData(data, {
                        amount: data ? data.fund_amount : undefined
                    });
                });
            });
            ...
            if (shouldOpenOnLoad) {
                const initialPayload = (defaultProgramId && payloadById[defaultProgramId]) ? payloadById[
                    defaultProgramId] : defaultPayload;
                openOverlayWithData(initialPayload, {
                    amount: defaultAmount
                });
            }
        });
