...
@section('content')
    ...
    <main class="flex-1">
        ...
        @if (session('success') || (old('program_source') === 'sponsorship_program' && $errors->any()))
            <div class="max-w-4xl mx-auto px-4 mt-4 space-y-3">
                ...
                @if (old('program_source') === 'sponsorship_program' && $errors->any())
        ...
        @php
            ...
            $formatProgramPayload = function ($program) {
                ...
                return [
                    ...
                    'remaining' => max((float) ($program->goal_amount ?? 0) - (float) ($program->raised_amount ?? 0), 0),
                    ...
                    'date_label' => $startDate && $endDate
                        ...
                        : ($startDate ? $startDate->format('M d, Y') : 'Schedule coming soon'),
                ];
            };
            ...
            $selectedProgramId = old('program_source') === 'sponsorship_program'
                ? old('sponsorship_program_id')
                : optional($defaultProgram)->id;

            $selectedProgram = $selectedProgramId
                ? $allPrograms->firstWhere('id', (int) $selectedProgramId)
                : null;
            ...
            if (old('program_source') === 'sponsorship_program') {
            ...
            $defaultAmountFormatted = $defaultAmount !== null
            ...
            $defaultDescription = $defaultPayload['description'] ?? 'Choose a program to learn more about the work it funds.';
            ...
            $shouldOpenSponsorshipModal = old('program_source') === 'sponsorship_program' && $errors->any();
        @endphp
                ...
                <div class="flex justify-between items-center pb-6">
                    ...
                    <div class="flex space-x-2">
                        <button id="prevBtn"
                            ...
                            <svg class="md:h-6 h-4 md:w-6 w-4 arrow-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"
                ...
                <div class="relative overflow-hidden">
                    @if ($ongoingPrograms->isEmpty())
                        <div class="bg-white/40 border border-[#DCCFD8] rounded-lg p-8 text-center text-[#91848C]">
                            There are no ongoing sponsorship programs at the moment.
                        </div>
                    @else
                        <div id="carousel" class="carousel-container flex overflow-x-auto scrollbar-hide">
                            @foreach ($ongoingPrograms as $program)
                                @php
                                    $image = asset('images/' . $sponsorshipImages[$loop->index % count($sponsorshipImages)]);
                                    $goal = (float) ($program->goal_amount ?? 0);
                                    $raised = (float) ($program->raised_amount ?? 0);
                                    $left = max($goal - $raised, 0);
                                    $payload = $formatProgramPayload($program);
                                    $payloadJson = $payload ? htmlspecialchars(json_encode($payload, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP), ENT_QUOTES, 'UTF-8') : '';
                                @endphp
                                    ...
                                    <div class="bg-[#F3E8EF] border border-[#DCCFD8] rounded-lg overflow-hidden hover:ring-2 ring-[#db69a2]">
                                        ...
                                        <div class="p-4 space-y-4">
                                            <div class="space-y-2">
                                                ...
                                                <p class="text-sm text-[#91848C] app-text">{{ Str::limit($program->description, 110) }}</p>
                                            </div>
                                            <button type="button"
        ...
        <div class="max-w-[97rem] mx-auto crousel-width">
            <div class="px-4 py-6 bg-[#F3E8EF] rounded-lg mt-8">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-2xl font-semibold text-[#213430] program-main">Upcoming Support Program</h2>
                </div>
                ...
                <div class="space-y-4">
                    @forelse ($upcomingPrograms as $program)
                        ...
                        <div class="bg-[#F3E8EF] rounded-lg p-4 mb-4 flex items-center justify-between md:flex hidden">
                            <div class="flex items-center">
                                ...
                                <div class="w-20 h-20 rounded-lg overflow-hidden mr-4">
                                    <img src="{{ $image }}" alt="{{ $program->title }}" class="w-full h-full object-cover" />
                                </div>
                                <div>
                                    <h3 class="text-xl font-semibold text-[#213430] mb-1 program-h">{{ $program->title }}</h3>
                                    <p class="text-sm text-[#91848C] program-p">{{ Str::limit($program->description, 140) }}</p>
                                </div>
                            </div>
                        ...
                        <div class="bg-[#F3E8EF] rounded-lg p-3 mb-4 flex items-center justify-between md:hidden flex">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <p class="text-[13px] font-light text-[#91848C]">{{ Str::limit($program->description, 110) }}</p>
                    ...
                    @empty
                        <div class="bg-white/40 border border-[#DCCFD8] rounded-lg p-8 text-center text-[#91848C]">
                            No upcoming sponsorship programs are scheduled right now.
                        </div>
                    @endforelse
                </div>
            </div>
        @if ($allPrograms->isNotEmpty())
            ...
            >
                <div class="relative w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden">
                    <button type="button" class="absolute right-4 top-4 rounded-full bg-white/80 p-2 text-[#213430] shadow-sm hover:bg-white" data-sp-close>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-[55%_45%]">
                        <div class="bg-[#FCEAF4] p-6 lg:p-8 space-y-6">
                            <div class="flex items-center gap-4">
                                <div class="flex flex-col items-center justify-center w-20 h-20 border-2 border-[#DB69A2] rounded-2xl bg-white">
                                    <span class="text-sm text-[#DB69A2]" data-sp-month>{{ $defaultMonthLabel }}</span>
                                    <span class="text-4xl font-bold text-[#DB69A2]" data-sp-day>{{ $defaultDayLabel }}</span>
                                </div>
                                <div class="space-y-2">
                                    <h2 class="text-2xl font-semibold text-[#213430]" data-sp-title>{{ $defaultTitle }}</h2>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div class="bg-white border border-[#E7CEDA] rounded-xl p-4">
                                    ...
                                    <p class="mt-1 text-lg font-semibold text-[#213430]" data-sp-goal>{{ $defaultGoalLabel }}</p>
                                </div>
                                <div class="bg-white border border-[#E7CEDA] rounded-xl p-4">
                                    <p class="text-xs uppercase tracking-wide text-[#B08BA2]">Raised</p>
                                    <p class="mt-1 text-lg font-semibold text-[#213430]" data-sp-raised>{{ $defaultRaisedLabel }}</p>
                                </div>
                                <div class="bg-white border border-[#E7CEDA] rounded-xl p-4">
                                    <p class="text-xs uppercase tracking-wide text-[#B08BA2]">Remaining</p>
                                    <p class="mt-1 text-lg font-semibold text-[#213430]" data-sp-remaining>{{ $defaultRemainingLabel }}</p>
                                </div>
                            </div>

                            <div class="bg-white border border-[#E7CEDA] rounded-xl p-5 text-sm text-[#7B5268] leading-relaxed" data-sp-description>{{ $defaultDescription }}</div>

                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-sm text-[#7B5268]">
                                <div>
                        ...
                        <div class="p-6 lg:p-8 space-y-6">
                            <form method="POST" action="{{ route('sponsor.sponsorships.store') }}" data-sp-form class="space-y-6">
                                @csrf
                                <input type="hidden" name="program_source" value="sponsorship_program">
                                <input type="hidden" name="sponsorship_program_id" value="{{ $selectedProgramIdValue }}" data-sp-input>
                                @error('sponsorship_program_id')
                                    <p class="text-xs text-[#B32020]">{{ $message }}</p>
                                @enderror

                                <div>
                                    <label for="sponsorship-program-amount" class="text-sm font-semibold text-[#213430]">Contribution amount</label>
                                    <div class="mt-2">
                                        <input
                                            id="sponsorship-program-amount"
                                            name="amount"
                                            type="number"
                                            min="1"
                                            step="0.01"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const applyPayload = function (payload, overrides = {}) {
                ...
                if (summaryEl) summaryEl.textContent = data.summary || 'Choose a program to learn more about the work it funds.';
                ...
                if (descriptionEl) descriptionEl.textContent = data.description || 'Choose a program to learn more about the work it funds.';
            ...
            triggers.forEach(function (trigger) {
                trigger.addEventListener('click', function (event) {
                    ...
                    openModal(parsed, { amount: parsed.remaining > 0 ? parsed.remaining : parsed.goal });
                });
            });
            ...
            if (openOnLoad && defaultPayload) {
                openModal(defaultPayload, { amountValue: amountInput ? amountInput.value : undefined });
            }
        });
    </script>
@endpush
