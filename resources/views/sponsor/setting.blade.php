                    ...
                    <div class="bg-[#F3E8EF] rounded-lg p-8">
                        ...
                        <form method="POST" action="{{ route('sponsor.settings.update') }}" enctype="multipart/form-data">
                                    ...
                                    <div class="w-28 h-28 rounded-full p-1">
                                        <div
                                            ...
                                            <img id="avatarPreview"
                                                src="{{ $profile && $profile->avatar ? '/storage/' . $profile->avatar : asset('images/profile.png') }}" alt="Profile"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label for="company_name" class="block text-[#213430] mb-1 app-text">Company Name</label>
                                    <input type="text" id="company_name" name="company_name"
                                        value="{{ old('company_name', $sponsorDetail->company_name ?? '') }}"
                                        class="w-full px-4 py-2 font-light rounded-md border border-[#DCCFD8] text-[#213430] bg-transparent focus:outline-none focus:ring-2 focus:ring-pink-300 app-text" />
                                </div>
                                <div>
                                    <label for="company_email" class="block text-[#213430] mb-1 app-text">Company Email</label>
                                    <input type="email" id="company_email" name="company_email"
                                        value="{{ old('company_email', $sponsorDetail->company_email ?? '') }}"
                                        class="w-full px-4 py-2 font-light rounded-md border border-[#DCCFD8] text-[#213430] bg-transparent focus:outline-none focus:ring-2 focus:ring-pink-300 app-text" />
                                </div>
                                <div>
                                    <label for="company_phone" class="block text-[#213430] mb-1 app-text">Company Phone</label>
                                    <input type="text" id="company_phone" name="company_phone"
                                        value="{{ old('company_phone', $sponsorDetail->company_phone ?? '') }}"
                                        class="w-full px-4 py-2 font-light rounded-md border border-[#DCCFD8] text-[#213430] bg-transparent focus:outline-none focus:ring-2 focus:ring-pink-300 app-text" />
                                </div>
                                <div>
                                    <label for="registration_number" class="block text-[#213430] mb-1 app-text">Registration Number</label>
                                    <input type="text" id="registration_number" name="registration_number"
                                        value="{{ old('registration_number', $sponsorDetail->registration_number ?? '') }}"
                                        class="w-full px-4 py-2 font-light rounded-md border border-[#DCCFD8] text-[#213430] bg-transparent focus:outline-none focus:ring-2 focus:ring-pink-300 app-text" />
                                </div>
                                <div>
                                    <label for="company_type" class="block text-[#213430] mb-1 app-text">Company Type</label>
                                    <select id="company_type" name="company_type"
                                        class="w-full appearance-none px-4 py-2 font-light rounded-md border border-[#DCCFD8] text-[#213430] bg-transparent focus:outline-none focus:ring-2 focus:ring-pink-300 app-text">
                                        <option value="">Select Company Type</option>
                                        <option value="LLC" @selected(old('company_type', $sponsorDetail->company_type ?? '') == 'LLC')>LLC</option>
                                        <option value="Corporation" @selected(old('company_type', $sponsorDetail->company_type ?? '') == 'Corporation')>Corporation</option>
                                        <option value="Partnership" @selected(old('company_type', $sponsorDetail->company_type ?? '') == 'Partnership')>Partnership</option>
                                        <option value="Sole Proprietorship" @selected(old('company_type', $sponsorDetail->company_type ?? '') == 'Sole Proprietorship')>Sole Proprietorship</option>
                                    </select>
                                </div>
                            </div>
                ...
                <div id="password" class="tab-content hidden">
                    <div class="bg-[#F3E8EF] rounded-lg p-6">
                        <h2 class="text-xl font-medium text-[#213430] border-b border-[#DCCFD8] pb-3 mb-4 app-main">Change
                            Password</h2>
                        <form method="POST" action="{{ route('sponsor.settings.password') }}">
                            @csrf
                            @method('PUT')
                            <div class="space-y-4">
                                <div>
                ...
                <div id="notifications" class="tab-content hidden">
                    <div class="bg-[#F3E8EF] rounded-lg p-6">
                        <h2 class="text-xl font-medium text-[#213430] border-b border-[#DCCFD8] pb-3 mb-4 app-main">Email &
                            SMS</h2>
                        <form method="POST" action="{{ route('sponsor.settings.notifications') }}">
                            @csrf
                            @method('PUT')
                            <div class="space-y-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" name="email_notification"
                                            {{ old('email_notification', $profile->email_notification ?? false) ? 'checked' : '' }} />
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <label class="switch">
                                        <input type="checkbox" name="sms_notification"
                                            {{ old('sms_notification', $profile->sms_notification ?? false) ? 'checked' : '' }} />
                                        <span class="slider"><span class="circle"></span></span>
                                    </label>
                                </div>
                                    ...
                                    <div class="space-y-2 md:space-y-0 md:space-x-4">
                                        <label class="flex items-center gap-2">
                                            <input type="checkbox" name="notify_on_new_notifications"
                                                {{ old('notify_on_new_notifications', $profile->notify_on_new_notifications ?? false) ? 'checked' : '' }}
                                        ...
                                        <label class="flex items-center gap-2">
                                            <input type="checkbox" name="notify_on_direct_message"
                                                {{ old('notify_on_direct_message', $profile->notify_on_direct_message ?? false) ? 'checked' : '' }}
                                                class="accent-[#DB69A2] w-4 h-4 border border-[#91848C] rounded appearance-none checked:appearance-auto focus:ring-0" />
                    ...
                    <div class="flex flex-wrap flex-center gap-6">
                        ...
                        <div class="w-full md:w-[49%] bg-[#F3E8EF] rounded-lg p-6">
                            <h2 class="text-xl font-medium text-[#213430] border-b border-[#DCCFD8] pb-3 mb-4 app-main">
                                Account Settings</h2>
                            <form method="POST" action="{{ route('sponsor.settings.account') }}">
                                @csrf
                                @method('PUT')
                                <div class="space-y-4">
                                    <div>
                        ...
                        <div class="w-full md:w-[49%] bg-[#F3E8EF] rounded-lg p-6">
                            <h2 class="text-xl font-medium text-[#213430] border-b border-[#DCCFD8] pb-3 mb-4 app-main">
                                Social Media</h2>
                            <form method="POST" action="{{ route('sponsor.settings.social') }}">
                                @csrf
                                @method('PUT')
                                <div class="space-y-4">
                                    <div>
